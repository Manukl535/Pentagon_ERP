...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit']) && isset($_POST['phone']) && isset($_POST['email'])) {
        ...
        $updateQuery = "UPDATE credentials SET otp = ? WHERE phone = ? AND email = ?";
        ...
        $getStoredOtpQuery = "SELECT otp FROM credentials WHERE phone = ? AND email = ?";
        ...
        if (strlen($password) < 6) {
            ...
            $updatePasswordQuery = "UPDATE credentials SET password = ? WHERE phone = ? AND email = ?";
